# Heddle Delivery Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Deliver Heddle as an agent-first, local-first temporal change-impact product that answers what changed, who/when, what is downstream-affected, and what must be re-verified without becoming a sibling-state aggregator, then prepare the post-admission federation integration work.

**Architecture:** Build a Python 3.12 package with a shared core library, SQLite temporal store, CLI, and first-class MCP stdio server over the same command handlers. Heddle consumes git as the hard dependency, consumes Loomweave/Legis/Filigree/Wardline/Charter only through their current published surfaces, and treats every sibling absence or mismatch as explicit degraded enrichment rather than a failure. Productization is gated by the spike evidence: without `spike/REPORT.md` and a `go` recommendation, implementation may produce a prototype but must not freeze federation contracts or patch sibling consumers. Heddle must never report a dated snapshot as current unless it has compared that snapshot commit to the current repo HEAD.

**Tech Stack:** Python 3.12, uv, stdlib `sqlite3`, stdlib `argparse`, dependency-free JSON-RPC MCP over stdio, pytest, ruff, mypy. SQLite is embedded and one DB per analyzed repo under a Heddle user-data directory.

**Prerequisites:**
- Work in a dedicated Heddle worktree or branch, with no new changes in `/home/john/filigree`, `/home/john/wardline`, `/home/john/legis`, `/home/john/loomweave`, or `/home/john/charter`.
...
- Owner admission is still out of scope for this repo. The plan can prepare integration tickets and fixtures; it cannot declare Heddle admitted.

---

## Source-Grounded Interface Facts

These facts were checked against local source and override Heddle prose where they differ.

- **Weft product doctrine:** `/home/john/weft/doctrine.md` says Weft is a federation, not a shared runtime/store/broker, and doctrine §7 reserves the future-product go/no-go test: one bounded authority, solo-useful, pairwise-sensible, suite-additive. `/home/john/weft/pm/product/decisions/0013-post-launch-priority-stack-and-discovery-pipeline.md` names Heddle as the next discovery slot and sets the standing admission bar: dogfood pain evidence, grep-test preference, enrich-only composition, doctrine fit, and hook-fed operation. `/home/john/weft/federation-sdk.md` requires SEI opacity, enrich-only behavior, and honest degradation when sibling capabilities are absent.
- **Weft Heddle status:** `/home/john/weft/members/heddle.md` says Heddle has no package, CLI, MCP server, or source implementation yet, and describes it as a design spike whose proposed authority is temporal/change-impact. `/home/john/weft/roadmap-ideas.md` says the Heddle idea survives only if it is bounded as temporal graph authority rather than a forbidden aggregator.
...
2. **Prototype slice:** Tasks 1-11 may build the standalone local-first Heddle prototype needed to generate spike evidence: package, store, git ingest, CLI, MCP, Loomweave probe/snapshots, blast radius, and reverify worklist. These tasks do not freeze federation contracts.
3. **Product slice:** after a `go` recommendation, run the productization gate and add release-candidate checks.
4. **Federation-integration preparation:** after a `go` recommendation, prepare Heddle-owned draft contract fixtures and sibling-owned consumer tickets. After owner admission, sibling repos can implement their consumers on their own trackers.
...
**Why this test:** The productization plan must be traceable to the live sibling and Weft sources, not only to Heddle's local design prose.
...
**Why minimal:** This creates a small, auditable source inventory. It does not copy sibling schemas into Heddle.
...
git commit -m "docs: record heddle source grounding"
```
...
### Task 1: Scaffold Heddle as a Python Package

**Files:**
- Create: `pyproject.toml`
- Create: `src/heddle/__init__.py`
- Create: `src/heddle/cli.py`
- Create: `src/heddle/mcp.py`
- Create: `src/heddle/errors.py`
...
```text
ModuleNotFoundError: No module named 'heddle'
```
...
```python
# src/heddle/__init__.py
__version__ = "0.1.0"
```

```python
# src/heddle/errors.py
from __future__ import annotations
...
```bash
git add pyproject.toml src/heddle tests/test_package.py
git commit -m "feat: scaffold heddle package"
```

**Definition of Done:**
- [ ] Tests fail before package exists.
- [ ] `heddle --version` and `python -m heddle.cli --version` work.
...
```text
ModuleNotFoundError: No module named 'heddle.store'
```
...
```bash
git add src/heddle/store.py tests/test_store.py
git commit -m "feat: add heddle temporal store"
```
...
**Files:**
- Create: `src/heddle/git.py`
- Modify: `src/heddle/store.py`
...
```text
ModuleNotFoundError: No module named 'heddle.git'
```
...
Add these concrete methods to `src/heddle/store.py`:
...
```bash
git add src/heddle/git.py src/heddle/store.py tests/test_git_backfill.py
...
**Files:**
- Create: `src/heddle/locators.py`
- Modify: `src/heddle/git.py`
...
```text
ModuleNotFoundError: No module named 'heddle.locators'
```

**Step 3: Implement locator extraction**

```python
# src/heddle/locators.py
from __future__ import annotations
...
Update `src/heddle/git.py` so `_name_status` rows for `.py` files at a commit read the file contents from that commit and emit one change event per locator:
...
```bash
git add src/heddle/locators.py src/heddle/git.py tests/test_locators.py tests/test_git_backfill.py
...
- [ ] Heddle still imports no sibling package.
...
**Files:**
- Create: `src/heddle/commands.py`
- Modify: `src/heddle/cli.py`
- Modify: `src/heddle/mcp.py`
...
Keep command logic out of CLI and MCP. `src/heddle/commands.py` owns JSON-shaped responses and both surfaces call it.

```python
# src/heddle/commands.py
from __future__ import annotations
...
Update `src/heddle/cli.py` in the same task so the CLI and MCP share the same handlers:
...
```bash
git add src/heddle/commands.py src/heddle/cli.py src/heddle/mcp.py tests/test_commands.py tests/test_mcp.py
...
```text
ModuleNotFoundError: No module named 'heddle.install'
```

**Step 3: Implement fail-soft hook installer**

```python
# src/heddle/install.py
from __future__ import annotations
...
def hook_body(executable: str) -> str:
    return f"""#!/bin/sh
# BEGIN HEDDLE MANAGED BLOCK
# Managed by Heddle. Fail-soft by design: Heddle must never block commits.
{executable} ingest-commit HEAD >/dev/null 2>&1 || true
# END HEDDLE MANAGED BLOCK
exit 0
"""


def install_hook(repo: Path, executable: str = "heddle") -> Path:
    hook = repo / ".git" / "hooks" / "post-commit"
    hook.parent.mkdir(parents=True, exist_ok=True)
    if hook.exists():
        existing = hook.read_text(encoding="utf-8")
        if "BEGIN HEDDLE MANAGED BLOCK" not in existing:
...
Add a single-commit ingest function to `src/heddle/git.py`:
...
Add a health-log method to `src/heddle/store.py`:
...
Replace `src/heddle/cli.py` with this command parser in the same task:
...
```bash
git add src/heddle/install.py src/heddle/cli.py src/heddle/git.py tests/test_hooks.py
...
- [ ] Heddle durable state still lives outside the working tree.
...
**Files:**
- Create: `src/heddle/loomweave.py`
...
**Why this test:** The first live integration step must distinguish "no Loomweave" from "Heddle broken" and must encode the tool names Heddle actually needs.
...
```text
ModuleNotFoundError: No module named 'heddle.loomweave'
```

**Step 3: Implement the probe**

Implement `src/heddle/loomweave.py` with this probe:
...
Run: `uv run heddle loomweave-probe --repo /home/john/loomweave --json`
...
```bash
git add src/heddle/loomweave.py tests/test_loomweave_probe.py spike/REPORT.md
git commit -m "feat: probe loomweave read surface"
```

**Definition of Done:**
- [ ] Probe distinguishes absent command, missing index, and serve failure.
- [ ] Probe names the exact Loomweave tools Heddle depends on.
...
```text
ModuleNotFoundError: No module named 'heddle.snapshot'
```

**Step 3: Implement snapshot recording**

```python
# src/heddle/snapshot.py
from __future__ import annotations
...
Add these methods to `src/heddle/store.py`:
...
```bash
git add src/heddle/store.py src/heddle/snapshot.py tests/test_snapshots.py
...
**Files:**
- Modify: `src/heddle/loomweave.py`
- Modify: `src/heddle/snapshot.py`
...
Add this pure parser to `src/heddle/snapshot.py`:
...
Add this MCP client wrapper to `src/heddle/loomweave.py`:
...
**Why minimal:** Heddle only needs dated edge snapshots. It must not read or expose "current structure" as its own answer.
...
```bash
git add src/heddle/loomweave.py src/heddle/snapshot.py tests/test_loomweave_snapshot_adapter.py tests/integration/test_loomweave_live.py spike/REPORT.md
...
- [ ] No query answers "who calls X right now" from Heddle.
...
Add this helper to `src/heddle/loomweave.py`:
...
Add a query helper to `src/heddle/store.py`:
...
```bash
git add src/heddle/loomweave.py src/heddle/store.py tests/test_sei_resolution.py
git commit -m "feat: resolve loomweave sei opaquely"
```

**Definition of Done:**
- [ ] Heddle stores SEIs only when Loomweave resolves them.
- [ ] Heddle treats SEI strings as opaque.
- [ ] Capability absence degrades to `sei: absent`.
- [ ] No Heddle code mints `loomweave:eid:*`.
...
**Files:**
- Create: `src/heddle/propagation.py`
- Modify: `src/heddle/commands.py`
- Modify: `src/heddle/mcp.py`
...
```text
ModuleNotFoundError: No module named 'heddle.propagation'
```

**Step 3: Implement traversal**

Use an in-memory adjacency list from the latest snapshot and keep a strict `depth <= 5` input guard. Add `src/heddle/propagation.py`:
...
Add the command wrapper to `src/heddle/commands.py`:
...
Extend `src/heddle/mcp.py` in the same task by adding this tool definition:
...
```bash
git add src/heddle/propagation.py src/heddle/commands.py src/heddle/mcp.py tests/test_propagation.py
...
```text
ModuleNotFoundError: No module named 'heddle.reverify'
```

**Step 3: Implement worklist renderer**

```python
# src/heddle/reverify.py
from __future__ import annotations
...
Extend `src/heddle/mcp.py` with this tool definition:
...
**Why minimal:** Test discovery is intentionally generic until Heddle has enough repository evidence to infer exact commands safely.
...
```bash
git add src/heddle/reverify.py src/heddle/commands.py src/heddle/cli.py src/heddle/mcp.py tests/test_reverify.py
...
**Why this test:** Enrich-only composition is easier to preserve when Heddle uses process/API/file surfaces instead of importing sibling internals.
...
git commit -m "test: guard heddle sibling boundaries"
```

**Definition of Done:**
- [ ] Heddle imports no sibling Python packages.
...
uv run heddle loomweave-probe --repo /home/john/loomweave --json

for repo in "${repos[@]}"; do
  uv run heddle backfill --repo "$repo" --json
  uv run heddle changed --repo "$repo" --json >/tmp/heddle-changed.json
done
...
import json
payload = json.load(open('/tmp/heddle-planted-changed.json'))
...
git commit -m "test: add heddle spike harness and report gate"
```
...
- Modify: `docs/plans/2026-06-12-heddle-delivery.md` if traceability gaps are found
...
```bash
git add tests/test_requirements_traceability.py docs/plans/2026-06-12-heddle-delivery.md
git commit -m "test: require heddle delivery traceability"
```
...
**Files:**
- Create: `src/heddle/productization.py`
...
```text
ModuleNotFoundError: No module named 'heddle.productization'
```

**Step 3: Write minimal implementation**

```python
# src/heddle/productization.py
from __future__ import annotations
...
Add a CLI command in `src/heddle/cli.py`:
...
```bash
git add src/heddle/productization.py src/heddle/cli.py tests/test_productization_gate.py
git commit -m "feat: gate heddle productization on spike findings"
```
...
### Task 15: Publish Heddle-Owned Federation Contracts

**Precondition:** Run `uv run heddle productization-gate --report spike/REPORT.md` and stop if it exits non-zero. These fixtures are Heddle-owned draft fixtures only; they are not normative Weft federation contracts until owner admission and glossary clearance.

**Files:**
- Create: `docs/federation/contracts.md`
- Create: `tests/fixtures/contracts/heddle/mcp-tool-inventory.json`
- Create: `tests/fixtures/contracts/heddle/mcp-response-changed.json`
- Create: `tests/fixtures/contracts/heddle/mcp-response-reverify.json`
- Create: `tests/contracts/test_heddle_contract_fixtures.py`
...
**Why this test:** A product entering the federation needs Heddle-owned fixtures for Heddle-owned surfaces; sibling repos should not reverse-engineer Heddle's wire shape from code.

**Step 2: Run test to verify it fails**

Run: `uv run pytest tests/contracts/test_heddle_contract_fixtures.py -v`

Expected output:

```text
FileNotFoundError: tests/fixtures/contracts/heddle/mcp-tool-inventory.json
```
...
```json
{
  "schema": "heddle.draft.mcp_tool_inventory.v1",
  ...
  "tools": [
    {
      ...
      "local_only": true,
      "peer_side_effects": [],
      "authority_boundary": "Traverses Heddle's dated snapshots; does not answer current graph truth."
    },
    {
      "name": "changed",
      "mutates": false,
      "local_only": true,
      "peer_side_effects": [],
      "authority_boundary": "Returns temporal change facts from git/Heddle store."
    },
    {
      "name": "reverify",
      "mutates": false,
...
```json
{
  "schema": "heddle.draft.changed.v1",
  "ok": true,
  "data": {
    "repo": "example",
...
```json
{
  "schema": "heddle.draft.reverify.v1",
  "ok": true,
  "data": {
    "repo": "example",
    "rev_range": "HEAD~1..HEAD",
...
**Why minimal:** These fixtures cover Heddle-owned read surfaces only. Post-admission sibling consumers pin against these fixtures and Heddle code, not against copied schema prose.
...
```bash
git add docs/federation/contracts.md tests/fixtures/contracts/heddle tests/contracts/test_heddle_contract_fixtures.py
git commit -m "docs: publish heddle federation contracts"
```

**Definition of Done:**
- [ ] Heddle-owned contract docs exist.
...
- [ ] No sibling-owned contract is copied or frozen in Heddle.

---

### Task 16: Prepare Post-Admission Consumer Ticket Package

**Precondition:** Run `uv run heddle productization-gate --report spike/REPORT.md` and stop if it exits non-zero. This task writes only Heddle-owned ticket-package documentation; it does not patch sibling repos.
...
**Why this test:** Product integration into the federation must become sibling-owned work after owner admission, not hidden patches bundled into Heddle.
...
and owner admission is explicit. Do not patch sibling repos from Heddle delivery
work.
...
git commit -m "docs: prepare heddle post-admission consumer tickets"
```
...
- [ ] No sibling patch is part of Heddle's delivery.
...
**Why this test:** The plan needs one operator command that proves Heddle is ready to be treated as a product candidate, not just a pile of passing unit tests.
...
git commit -m "test: add heddle product release candidate gate"
```
...
| CON-ORG-03 naming placeholder | Tasks 12, 15 | `tests/contracts/test_heddle_contract_fixtures.py`, `scripts/run_spike.sh` |
| CON-ORG-04 hook-fed | Task 5 | `tests/test_hooks.py` |
| Productization from spike findings | Task 14 | `heddle productization-gate`, `tests/test_productization_gate.py` |
| Federation contract preparation | Task 15 | Heddle-owned fixtures and `tests/contracts/test_heddle_contract_fixtures.py` |
...
- Do not use Heddle to answer current structure. Route current graph questions to Loomweave.
- If an agent finds the flow annoying, fix the flow. Friction in `heddle changed`, `heddle reverify`, MCP `tools/list`, or first-run setup is a product defect, not a documentation issue.
