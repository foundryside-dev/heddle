...
2. Federation integration is a priority. Warpline must consume sibling capabilities only through published surfaces and prepare clean post-admission integration paths.
3. Usability is a hard requirement. If Warpline is harder to use than the current manual/grep-based workflow, the implementation has failed. Deficiencies in the MCP interface are grounds for major refactor, not minor polish.
...
5. Warpline must not become a forbidden aggregator or shared federation store.
...
- Whether Warpline is actually easier and more useful than current workflows.
...
- Do not patch sibling repos as part of Warpline implementation unless explicitly authorized after owner admission.
- Draft integration contracts and consumer tickets must stay Warpline-owned and pre-admission until explicitly accepted.
...
- Use SQLite safely and keep Warpline state outside analyzed repos.
...
Do not stop at partial implementation. Continue through plan, implement, review, fix, and verify loops until Warpline is genuinely ready as a product candidate or until a documented no-go/park decision is reached from evidence.
```
