# PDR-0001 - Move Warpline To Product-Candidate Ownership
...
A later live review found that Warpline is a disciplined solo-mode prototype but
not product-ready or member-grade. This does not reverse the ownership posture;
it narrows it. Warpline remains a first-class product bet, but productization and
...
Warpline has a useful prototype scaffold, core CLI/MCP surfaces, honest
`NO_SNAPSHOT` behavior, reverify worklists, and Warpline-owned draft federation
contracts. The remaining risk is no longer "can a prototype exist"; it is
whether Warpline is treated as a coherent product whose MCP surface is at least as
...
1. Keep Warpline as a spike-only architecture workspace.
   - Pro: avoids overclaiming admission.
   - Con: leaves product intent, metrics, and MCP UX bar implicit.
2. Move Warpline to a product-candidate ownership posture.
...
3. Declare Warpline an admitted Weft member now.
   - Pro: gives the design a clear label.
   - Con: violates the owner-reserved admission boundary.

## The call

Move Warpline to product-candidate ownership posture. Warpline is treated as a
first-class product candidate inside this repo, with MCP and federation design
as primary product surfaces. Warpline is not admitted to the Weft federation by
this decision.
...
good as existing tools in solo mode and better with federation members, Warpline
fails as a product candidate no matter how much implementation exists. It also
makes sibling boundaries testable by keeping Warpline-owned contracts and consumer
...
- Warpline implementation requires patching a sibling repo before owner admission.
- The owner rejects Warpline admission after reviewing the product-candidate
  evidence.
