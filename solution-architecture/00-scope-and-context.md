...
supervision load the federation exists to remove (PDR-0004, PDR-0013). Warpline is the
...
- scope_tier: M — trigger (a): new bounded context, new deployment unit, and new data owner (Warpline owns temporal change data no member owns today)
...
- Change *execution* / rollback — the Shuttle thought-bubble's gap, not Warpline's
- Agent identity/trust — Tabard (Later); Warpline records actor strings as given, it does not verify them
...
2. `[ASSUMED]` SEIs are stable across the commit ranges Warpline ingests (the SEI standard is LOCKED, but per-entity SEI continuity through renames/moves is exactly what the spike must measure).
...
3. Where exactly is the Warpline/Charter seam? Proposed: Warpline = structural/temporal slice, Charter = obligations slice, Charter consumes Warpline. Needs Charter-side confirmation post-launch. (Spike Q3.)
