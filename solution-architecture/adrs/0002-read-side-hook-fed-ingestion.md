...
**Context:** Warpline needs change events and structural edges. Three candidate
acquisition shapes: (a) siblings push events to Warpline; (b) agents log changes
to Warpline as discipline; (c) Warpline pulls — git hooks + reading published
sibling surfaces.

**Decision:** Shape (c). Cold backfill walks `git log`; a post-commit hook
appends incremental events; when a Loomweave surface is readable, Warpline reads
...
- Doctrine §5/§6: (a) makes siblings *participants* in Warpline's function
...
enrichment a member chooses to add in its own tracker, never required by Warpline.

**Consequences:** Warpline's edge data is as fresh as its last read of the
