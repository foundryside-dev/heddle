# ADR-0004 — The aggregator firewall: Warpline owns "over time," never "now"
...
aggregator") and the reason Warpline is a spike, not a build.

**Decision:** A hard firewall on what the Temporal Store may contain and what
queries Warpline may answer:
...
1. **Stored:** only temporal facts no sibling stores — append-only change
   ...
   re-read; Warpline's store is never the recovery source for anyone else's data.
...
**Rollback / expiry:** if dogfood shows agents need Warpline to answer
