...
DEFAULT_DOGFOOD_RESULTS = Path("/tmp/warpline-dogfood-results.json")
SOLO_THRESHOLD = 8
...
def run_dogfood_evaluator(
    *,
    ...
    root = work_dir or Path("/tmp/warpline-dogfood-work")
    if root.exists():
        shutil.rmtree(root)
...
def _run_case(root: Path, case: DogfoodCase, *, lane: str) -> dict[str, Any]:
    ...
    with WarplineStore.open(default_store_path(repo)) as store:
        ...
        if lane == "federation":
            capture_edge_snapshot(
                ...
                source_version="warpline-dogfood-draft-spec",
            )
    ...
    reverify_args = changed_response["next_actions"]["warpline_reverify_worklist_get"]["arguments"]
...
def _run_real_member_case(root: Path, source_repo: Path) -> dict[str, Any]:
    ...
    try:
        ...
        reverify_action = changed_payload["next_actions"]["warpline_reverify_worklist_get"]
        reverify_args = reverify_action["arguments"]
        reverify_payload = _call_tool_stdio("reverify", reverify_args)
        tool_calls += 1
        reverify_data = reverify_payload["data"]

        warpline_paths = _changed_paths(changed_data)
        ...
        return {
            ...
            "warpline_answer": {
                ...
                "changed_entity_key_ids": changed_key_ids,
                "changed_paths": sorted(warpline_paths),
                "reverify_completeness": reverify_data.get("completeness"),
...
def _select_real_member_rev_range(repo: Path) -> tuple[str, str]:
    ...
    for sha in commits:
        ...
        reverify_action = changed_env["next_actions"]["warpline_reverify_worklist_get"]
