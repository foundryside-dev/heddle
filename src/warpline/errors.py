...
# FROZEN warpline.error.v1 vocabulary (see
# /home/john/weft/pm/2026-06-13-warpline-interface-lock.md "canonical error envelope").
...
class WarplineError(Exception):
    """Base for the FROZEN warpline.error.v1 recoverable-error contract.
...
class MissingRequiredFieldError(WarplineError):
    code = "missing_required_field"
    retryability = "retry_with_changes"
    hint = "Supply the required argument and retry the same tool."


class InvalidRepoError(WarplineError):
    code = "invalid_repo"
    rejected_field = "repo"
    retryability = "retry_with_changes"
    hint = "Pass an absolute path to a git repository warpline can read."
...
class InvalidEntityRefError(WarplineError):
...
class InvalidChangedRefsError(WarplineError):
...
class InvalidDepthError(WarplineError):
    code = "invalid_depth"
...
class InvalidFilterError(WarplineError):
    code = "invalid_filter"
...
class InvalidSortError(WarplineError):
    code = "invalid_sort"
    retryability = "retry_with_changes"
    hint = "Pass a recognised sort_by/sort_order for this tool."


class PeerUnavailableError(WarplineError):
    code = "peer_unavailable"
    retryability = "retry_safe"
    hint = "A federation peer was unreachable; retry once the peer is available."


class SnapshotUnavailableError(WarplineError):
    code = "snapshot_unavailable"
    retryability = "retry_with_changes"
    hint = "Run warpline_edge_snapshot_capture before requesting graph-enriched output."


class InternalError(WarplineError):
    code = "internal_error"
    retryability = "fatal"
    hint = "Inspect server logs before retrying; this is a warpline defect."
