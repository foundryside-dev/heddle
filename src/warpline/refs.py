...
from warpline.errors import InvalidChangedRefsError, InvalidEntityRefError
...
def entity_view(
    ...
    *,
    ...
    Every warpline-outbound entity carries ``locator`` and ``sei`` (``sei: null``
    when warpline has not resolved one). ``warpline_entity_key_id`` is internal and
    ...
    if include_key_id:
        view["warpline_entity_key_id"] = row.get("id") or row.get("entity_key_id")
    if path is not None:
        view["path"] = path
    return view
