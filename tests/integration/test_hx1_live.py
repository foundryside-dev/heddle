...
from warpline.loomweave import LoomweaveMcpClient, LoomweaveProbe, resolve_sei_for_locator

WARPLINE_REPO = Path(__file__).resolve().parents[2]  # repo root, whatever it is named
...
_LIVE_LOCATORS = [
    "python:function:src/warpline/store.py::WarplineStore.timeline",
    "python:function:src/warpline/locators.py::python_entity_locators",
    "file:src/warpline/store.py",
]


def _live_client() -> LoomweaveMcpClient:
    probe = LoomweaveProbe(repo=WARPLINE_REPO).probe()
    if probe.get("status") != "available":
        pytest.skip(f"loomweave unavailable for live HX1 probe: {probe}")
    return LoomweaveMcpClient(repo=WARPLINE_REPO)
...
def test_hx1_unindexed_locator_degrades_to_none_not_error() -> None:
    ...
    sei = resolve_sei_for_locator(
        client, "python:function:src/warpline/does_not_exist.py::nope"
    )
    assert sei is None
